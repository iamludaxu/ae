 ...
 * 下拉刷新和加载更多
 */
public class PullAndLoadMoreRefreshRecyclerView extends RecyclerView {

    private static final int STATE_LOADING_MORE = 3;
    private static final int STATE_PULL_TO_REFRESH = 0;
    private static final int STATE_LOADING = 1;
    private static final int STATE_RELASE_TO_REFRESH = 2;
    private PullAndLoadMoreRecyclerViewAdapter mAdapter;
    ...
    public PullAndLoadMoreRefreshRecyclerView(Context context) {
        this(context, null, 0);
    }

    public PullAndLoadMoreRefreshRecyclerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PullAndLoadMoreRefreshRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    protected void onAttachedToWindow() {
        ...
        if (!(adapter instanceof PullAndLoadMoreRecyclerViewAdapter)) {
            throw new IllegalArgumentException("the adapter must extents BaseRefreshRecyclerViewAdapter");
        }
        mAdapter = (PullAndLoadMoreRecyclerViewAdapter) adapter;
        mAdapter.setFooterClickListener(new PullAndLoadMoreRecyclerViewAdapter.FooterClickListener() {
            @Override
